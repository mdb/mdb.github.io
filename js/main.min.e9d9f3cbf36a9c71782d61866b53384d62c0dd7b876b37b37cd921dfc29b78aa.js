document.addEventListener("DOMContentLoaded",()=>{function t(e){e.style.transition="opacity 2s",e.style.opacity="1"}function n(){const e=document.getElementsByTagName("img");for(let n of e)n.complete&&t(n),n.addEventListener("load",()=>{t(n)})}function i(e){const t=document.querySelectorAll("ul.ig-feed"),n=e.data.map(e=>`
          <li class="item">
            <a class="thumbnail" href="${e.permalink}">
              <img src="${e.github_media_url}" />
            </a>
          </li>`).join("");return t.forEach(e=>e.innerHTML=e.innerHTML+n),e}function l(e){const i="button#load-more",s=document.querySelector("body.instagram main");let t=document.querySelector(i);if(!s)return!1;t&&t.remove(),s.innerHTML=`${s.innerHTML}<button id="load-more">Load more</button>`,t=document.querySelector(i);const a="url";t.setAttribute(a,e.paging.next),t.addEventListener("click",e=>{e.stopPropagation();const t=new URL(e.target.getAttribute(a));o(`${t.protocol}//mikeball.info/${t.pathname}`).then(n)})}function o(e){return e=e||"https://mikeball.info/feeder/feeds/instagram-media-0.json",fetch(e).then(e=>e.json()).then(i).then(l)}function a(e){const t=document.querySelectorAll("ul.store-feed"),n=e.slice(0,4).map(e=>{const t=`https://tiendah.bigcartel.com/${e.url}`;return`
          <li class="item">
