...
* The [Grafana Terraform provider](https://registry.terraform.io/providers/grafana/grafana/latest/docs) offers resources for managing many other aspects of Grafana too, such as teams, API keys, etc. A real-world example might manage more aspects of Grafana via Terraform, such as dynamically create a unique Grafana API key for each git repository in the targeted organization and seed a corresponding `GRAFANA_API_KEY` [GitHub Actions secret](https://registry.terraform.io/providers/integrations/github/latest/docs/resources/actions_secret) in each repository, such that the repositories' GitHub Actions workflow can programmatically interact with Grafana, enabling further platform automation capabilities.
